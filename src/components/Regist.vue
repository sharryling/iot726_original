		...
		<div class="regist-box">
			...
			<form>
				<div class="ipunt-wrap">
					...
					<input type="text" id="username" placeholder="账户" v-model="username">
				</div>
<!-- 				<div class="ipunt-wrap">
				...
				<div class="ipunt-wrap">
  				...
  				<div class="ipunt-wrap">
					<label for="company" class="icon-password"></label>
					<input type="text" id="company" placeholder="公司" v-model="company_reg">
				</div>
 				<div class="ipunt-wrap">
					<label for="email" class="icon-password"></label>
					<input type="text" id="email" placeholder="邮箱" v-model="email_reg">
				</div>
...
export default {
	...
	methods: {
		...
		Regist() {
			if (!this.username || !this.company_reg || !this.password || !this.confimpassword || !this.email_reg) {
				this.dialog = true
		...
		/* 	if (!(/^1[34578]\d{9}$/).test(this.phone)) {
			...
			var user = {
				...
				password: this.password.MD5(32),
				email: this.email_reg,
				company:this.company_reg
			}
			$.ajax({
				url: 'https://cloudapi.usr.cn/usrCloud/user/regUser',
				type: 'post',
				dataType: 'json',
...
.regist-box .logo{
	...
	height: 120px;
	margin: 40px;
	background: url('../assets/images/jj1.png') no-repeat;
	/* background: red; */
	background-size: cover;
}
.ipunt-wrap label{
